...
import sys
import socket  # Re-added since it's required for SOURCE_ID
import json
import requests
import tempfile
import subprocess
...
monitor_thread = None
DUNGEON_NAMES = {}  
CURRENT_VERSION = "v1.0.3"  # Update this when you release a new version

# GitHub Info
GITHUB_REPO = "https://api.github.com/repos/PoviP/AK-monitoring/releases/latest" 
...
def check_for_updates():
    """Check for updates and notify the user to run the batch file."""
    try:
        response = requests.get(GITHUB_REPO, timeout=10)  # Replace this with your GitHub repo API link
        if response.status_code == 200:
            ...
            if latest_version > CURRENT_VERSION:
                # Notify the user
                batch_file_path = os.path.join(os.path.dirname(__file__), "update_and_restart.bat")
                messagebox.showinfo(
                    "Update Available",
                    f"A new version ({latest_version}) is available.\n\n"
                    "Please close the application and run the batch file located here:\n"
                    f"{batch_file_path}"
                )
            else:
                print("You are running the latest version.")
        else:
            print(f"Failed to check for updates. HTTP Code: {response.status_code}")
...
def create_tray_icon(root):
    ...
    draw.rectangle((16, 8, 48, 56), fill=(50, 180, 220))
    draw.rectangle((28, 40, 36, 60), fill=(30, 100, 150))
    ...
    def on_show_window(icon):
        icon.stop()
        root.after(0, setup_tray_icon)
        root.deiconify()
    ...
    icon = pystray.Icon("AstralKeys Monitor", image, "AstralKeys Monitor", menu)
...
def get_credentials():
    ...
    try:
        # Check several possible locations for the service account file
        possible_locations = [
            os.path.join(os.path.dirname(sys.executable), 'service-account.json'),  # Same folder as exe
            os.path.join(os.getcwd(), 'service-account.json'),                     # Current working directory
            os.path.join(os.path.expanduser("~"), 'astralkeys', 'service-account.json'),  # User's home dir in astralkeys subfolder
            resource_path('service-account.json')                                  # Inside the package (fallback)
        ]
        
        service_account_file = None
        for location in possible_locations:
            print(f"Checking for service account file at: {location}")
        ...
        if not service_account_file:
            print("Service account file not found in any of the expected locations!")
            messagebox.showerror("Error", 
                "service-account.json not found! Please place the service-account.json file in the same folder as the executable or in your home directory's 'astralkeys' folder.")
            return None
...
def setup_service_account():
    """Guide the user to set up a service account file"""
    try:
        # Ask user to select the service account JSON file
        file_path = filedialog.askopenfilename(
            title="Select your Google Service Account JSON file",
        ...
        # Create a directory to store the file if needed
        target_dir = os.path.join(os.path.expanduser("~"), 'astralkeys')
        ...
        messagebox.showinfo("Success", 
            f"Service account file has been set up successfully at:\n{target_path}")
        return True
        
    except Exception as e:
        print(f"Error setting up service account: {e}")
        messagebox.showerror("Error", f"Failed to set up service account: {str(e)}")
        return False
...
def create_gui():
    """Create the GUI"""
    global monitoring_active, monitor_thread, SPREADSHEET_ID, root, tray_icon
    
    # Create config path
    config_path = os.path.join(os.path.expanduser("~"), "astralkeys_config.json")
    
    # Define on_close function BEFORE creating the main window
    def on_close():
        if monitoring_active:
            # If monitoring is active, minimize to tray instead of closing
            root.withdraw()
            setup_tray_icon()
        else:
            # If not monitoring, actually close the application
            root.destroy()
    ...
    root.protocol("WM_DELETE_WINDOW", on_close)  # Now on_close is defined
    ...
    tk.Label(status_frame, text=f"Version: {CURRENT_VERSION}").pack(side=tk.RIGHT)
    ...
    tk.Button(button_frame, text="Minimize to Tray", command=minimize_to_tray).pack(side=tk.LEFT, padx=5)
    
    # service account button
    tk.Button(button_frame, text="Setup Service Account", command=setup_service_account).pack(side=tk.LEFT, padx=5)
    ...
    if args.autostart:
        ...
        if settings_loaded:
            ...
            if file_path and os.path.isfile(file_path):
                ...
                # If autostart, minimize to tray after starting monitoring
                def minimize_after_start():
                    if monitoring_active:
                        minimize_to_tray()
                
                root.after(5000, minimize_after_start)
